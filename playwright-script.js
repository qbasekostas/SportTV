    ...
    try {
        ...
        for (const targetUrl of targetUrls) {
            ...
            try {
                ...
                console.log("\x1b[33m Page loaded with status:\x1b[0m", response.status(), targetUrl);

                // Extract Referer header automatically
                const referer = response.request().headers()['referer'] || targetUrl;
                console.log("\x1b[32mDetected Referer:\x1b[0m", referer);
                ...
                try {
                    decodedM3U8 = await page.evaluate(() => {
                        ...
                        throw new Error('Could not get the M3U8 URL from player.options.source');
                    });
