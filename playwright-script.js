           ...
           page.on('pageerror', (err) => {
            ...
            try {
                ...
                const response = await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 5000 });
                 ...
                 // Dynamically Get Referer
                const referer = await page.evaluate(()=> document.referrer || 'https://foothubhd.org/');
                 console.log("\x1b[32mReferer: \x1b[0m", referer);

              page.on('response', async (response) => {
                    const url = response.url();
                    if (!response.ok()) {
                        console.log("\x1b[33mResponse failed for url: \x1b[0m", url);
                        return;
                    }
                    try{
                        const content = await response.text();
                         const genericMatch = content.match(/source:\s*window\.atob\('(.*?)'\)/);
                           if (genericMatch && genericMatch[1]) {
                             const decodedM3U8= Buffer.from(genericMatch[1], 'base64').toString('utf-8');
                             const streamName = new URL(decodedM3U8).pathname.split('/').slice(-2, -1)[0];
                            m3u8Links.add({ streamName, url: decodedM3U8, referer });
                             console.log(`\x1b[32mFound .m3u8 URL in XHR response:\x1b[0m`,decodedM3U8);
                           }
                       }
                       catch(e){
                           console.log("\x1b[33mCould not get the M3U8 url from response body. \x1b[0m", url, e);
                       }
                  });
