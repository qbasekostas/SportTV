                  ...
                  // Wait for a specific Clappr video element
                   try {
                     ...
                     await page.waitForSelector(videoSelector, { timeout: CLAPPR_TIMEOUT });
                        ...
                        console.log("\x1b[33m Timeout waiting for Clappr video element.\x1b[0m", targetUrl);
                         await page.screenshot({ path: `error_screenshot_${Date.now()}.png` });
                       continue; // Skip the page and continue with the next.
                    }

                 //Attempt to move mouse and click play button to force the XHR
                 try {
                   const videoElement = await page.$('#player > div > div.jw-media.jw-reset > video');
                  await videoElement.hover();
                   ...
                   console.log("\x1b[32mClicked Play button\x1b[0m");
                    ...
                    try{
                        const m3u8Response = await page.waitForResponse(response => response.url().endsWith('.m3u8'),{timeout: 10000});
                        const url = m3u8Response.url();
                        const referer = m3u8Response.request().headers()['referer'] || 'N/A';
                        ...
                        console.log("\x1b[33mTimeout waiting for XHR response for m3u8 \x1b[0m");
                       // Fallback to generic regex if all else fails.
