        ...
        for (const targetUrl of targetUrls) {
           ...
           await page.setExtraHTTPHeaders({
            ...
            page.on('pageerror', (err) => {
                  console.log("\x1b[31m Javascript error:\x1b[0m", err.message, targetUrl)
              });
            ...
            try {
                ...
                // Initialize the console-ban script with some specific options to prevent redirection
                  ...
                  // Wait for a specific Clappr UI element
                   try{
                       const mediaControlSelector = 'div[data-player] div[data-media-control]';
                      console.log("\x1b[35m Waiting for Clappr Media Control element:", mediaControlSelector,'\x1b[0m');
                       await page.waitForSelector(mediaControlSelector, { timeout: CLAPPR_TIMEOUT });
                       console.log("\x1b[32m Clappr media control element found.\x1b[0m");
                   }
                    catch(e){
                      console.log("\x1b[33mTimeout waiting for media control element. Attempting manual Clappr initialization. \x1b[0m");
                       try {
                            await page.evaluate(() => {
                                 if (typeof Clappr !== 'undefined' && document.querySelector('#player > div > script')) {
                                   console.log('\x1b[33m Executing Clappr init manually \x1b[0m');
                                   const playerDiv = document.getElementById('player');
                                   const scriptContent = document.querySelector('#player > div > script').textContent;
                                      eval(scriptContent);
                                  }else{
                                    console.log('\x1b[33m Could not initialize Clappr manually. \x1b[0m');
                                }
                            });
                            try{
                               ...
                               console.log("\x1b[33mTimeout waiting for video element after manual Clappr initialization.\x1b[0m");
                            }
                       }catch(e2){
                           console.log('\x1b[33m Could not load the Clappr player script or initialize Clappr manually:\x1b[0m', e2);
                         }
