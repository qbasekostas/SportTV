            ...
            try {
                 ...
                 try{
                   ...
                   console.log("\x1b[35m Waiting for Clappr Video element:", videoSelector,'\x1b[0m');
                    ...
                    // Extract Child M3U8 files
                     const childM3U8s = await page.evaluate((decodedM3U8) => {
                        const extractLines = (m3u8Content, filePattern) => {
                        ...
                        return urls;
                         ...
                         const m3u8Urls = extractLines(decodedM3U8, /\.m3u8/);
                          return m3u8Urls;
                      },decodedM3U8);
                     childM3U8s.forEach(childUrl=>{
                        const streamName = new URL(childUrl).pathname.split('/').slice(-2, -1)[0];
                          m3u8Links.add({ streamName, url: childUrl, referer: 'https://foothubhd.org/' });
                        console.log(`\x1b[32mFound .m3u8 Sub-playlist URL:\x1b[0m ${childUrl}`);
