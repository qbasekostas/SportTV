      ...
      await page.setExtraHTTPHeaders({
       ...
       try {
          ...
          await page.goto(targetUrl, { waitUntil: 'networkidle2' });
                ...
                let videoElement;
               try {
                    const videoElements = await page.$x('//video | //video[@data-html5-video]');
                      if (videoElements.length > 0) {
                         videoElement = videoElements[0];
                           console.log(`\x1b[34mvideo element found (xpath): \x1b[0m${targetUrl}`);
                        
                            const hasDataHtml5Video = await page.evaluate((el) => el.hasAttribute('data-html5-video'),videoElement);
                           if(!hasDataHtml5Video){
                            ...
                            console.error("\x1b[31mNo video element found (xpath):\x1b[0m", targetUrl);
                          }
                  } catch(e){
